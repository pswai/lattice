...
Session routes (`/auth/*`, `/workspaces/*`) use the `lt_session` cookie set by login/signup.
...
Session-based authentication for the SaaS UI.
...
```json
{
  ...
  "email_verification_token": "token-if-dev-mode"
}
```

Sets `lt_session` cookie. Sends verification email if email provider is configured.
...
Sets `lt_session` cookie. Returns 401 on invalid credentials.

### POST /auth/logout

Revokes the current session. Clears the session cookie.
...
**Response:** 204 No Content on success, 400 if token is invalid/expired.

### GET /auth/oauth/github

Redirects to GitHub OAuth authorization page. Requires `GITHUB_OAUTH_CLIENT_ID` to be configured.

### GET /auth/oauth/github/callback

GitHub OAuth callback. Exchanges code for token, creates/finds user, sets session cookie, redirects to `APP_BASE_URL`.
...
{
  "workspaces": [
    {"workspace_id": "my-team", "name": "My Team", "role": "owner", "joined_at": "..."}
  ]
}
```
...
```json
{
  "invitation_id": "uuid",
  "expires_at": "...",
  "invite_token": "token-if-dev-mode"
}
```

### GET /workspaces/:id/invites

List pending invitations. Requires `owner` or `admin` role.

### DELETE /workspaces/:id/invites/:invId

Revoke an invitation. Requires `owner` or `admin` role.
...
**Response (201):**
```json
{"workspace_id": "my-team", "role": "member"}
```
...
```json
[
  {"user_id": "uuid", "email": "user@example.com", "name": "Name", "role": "owner", "joined_at": "..."}
]
```
...
Remove a member (or leave the workspace). Owner can remove others; any member can remove themselves.

### GET /workspaces/:id/audit

Query the audit log for this workspace. Any member can read.

**Query params:** `actor`, `action`, `resource`, `since`, `until`, `limit`, `cursor`
...
Get current usage and plan limits for a workspace.

**Response (200):**
```json
{
  "period": "2025-01",
  "usage": {"exec_count": 42, "api_call_count": 1500, "storage_bytes": 50000},
  "limits": {
    "plan_id": "free",
    "plan_name": "Free",
    "exec_quota": 100,
    "api_call_quota": 10000,
    "storage_bytes_quota": 104857600,
    "seat_quota": 5
  },
  "soft_warning": false,
  "hard_exceeded": false
