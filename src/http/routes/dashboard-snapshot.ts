...
export function createDashboardSnapshotRoutes(db: DbAdapter): Hono {
  ...
  router.get('/', async (c) => {
    ...
    // Optional sections filter: ?sections=agents,tasks,events,analytics,auditLog,apiKeys
    const sectionsParam = c.req.query('sections');
    const want = sectionsParam
      ? new Set(sectionsParam.split(',').map((s) => s.trim()))
      : null; // null = all sections (backward compat)
    const include = (key: string) => !want || want.has(key);

    const [agents, tasks, events, analytics, workspace, auditRows, apiKeys] = await Promise.all([
      include('agents')
        ? db.all<{
            id: string; workspace_id: string; capabilities: string; status: string;
            metadata: string; last_heartbeat: string; registered_at: string;
          }>('SELECT * FROM agents WHERE workspace_id = ? ORDER BY registered_at DESC', workspaceId)
        : Promise.resolve(null),

      include('tasks')
        ? db.all<{
            id: number; workspace_id: string; description: string; status: string;
            result: string | null; created_by: string; claimed_by: string | null;
            claimed_at: string | null; version: number; priority: string;
      ...
      include('events')
        ? db.all<{
            id: number; workspace_id: string; event_type: string; message: string;
      ...
      // Workspace name is always fetched (lightweight, needed by header)
      ...
      include('auditLog')
        ? db.all<{
            id: number; actor: string; action: string; resource_type: string | null;
            resource_id: string | null; metadata: string; ip: string | null; created_at: string;
          }>(
      ...
      include('apiKeys')
        ? db.all<{
            id: number; label: string; scope: string; created_at: string;
